...
module reg_file #(parameter DATA_WIDTH=8, ADDR_WIDTH=2)
  ...
  input  logic [2*DATA_WIDTH-1:0] w_data; // changed it to 2 * DATA_WIDTH
  output logic [2*DATA_WIDTH-1:0] r_data; // 2 * DATA_WIDTH
  
  // array declaration (registers)
  logic [2*DATA_WIDTH-1:0] array_reg [0:2**ADDR_WIDTH-1]; // 2 * DATA_WIDTH
